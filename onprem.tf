...
resource "aws_instance" "ace-onprem-partner-csr" {
  ...
  user_data                   = <<EOF
    ...
    ios-config-1010 = "crypto keyring OnPrem-Partner-Aviatrix"
    ...
    ios-config-1130 = "crypto ipsec transform-set OnPrem-Partner-Aviatrix esp-aes 256 esp-sha256-hmac"
    ios-config-1140 = "mode tunnel"
    ios-config-1150 = "crypto ipsec df-bit clear"
    ios-config-1160 = "crypto ipsec profile OnPrem-Partner-Aviatrix"
...
output "onprem_partner_ubu_public_ip" {
  value = module.ace-onprem-ubu.public_ip
}

output "onprem_partner_ubu_private_ip" {
